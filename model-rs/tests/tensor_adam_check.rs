...
fn weights_match_after_one_step() {
    ...
    let tc = TrainConfig { n_steps: 10, ..TrainConfig::default() };
    ...
    let max_diff: f64 = s_wte0.iter().zip(&t_wte0).map(|(a, b)| (a - b).abs()).fold(0.0, f64::max);
    ...
    let max_lm: f64 = s_lm0.iter().zip(&t_lm0).map(|(a, b)| (a - b).abs()).fold(0.0, f64::max);
    ...
    let max_wpe: f64 = s_wpe0.iter().zip(&t_wpe0).map(|(a, b)| (a - b).abs()).fold(0.0, f64::max);
    println!("  Max wpe[0] diff: {max_wpe}");

    assert!(max_diff < 1e-14, "wte weights diverge after Adam: {max_diff}");
}
